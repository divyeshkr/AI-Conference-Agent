...
from core.config import PROVIDERS, get_settings, update_settings
...
with tab_model:
    ...
    with c1:
        provider = st.selectbox(
            "Provider", PROVIDERS, index=PROVIDERS.index(settings.provider)
        )
    ...
    with c2:
        text_model = st.text_input("Text model / deployment", value=settings.text_model)
        vision_model = st.text_input("Vision model / deployment", value=settings.vision_model)
        api_version = st.text_input(
            "Azure API version", value=settings.api_version, disabled=provider != "azure"
        )

    if st.button("Save and test", type="primary"):
        update_settings(
            provider=provider,
    ...
    with st.expander("Persist these settings in a .env file"):
        st.code(
            "CI_PROVIDER=azure\n"
            "AZURE_OPENAI_API_KEY=...\n"
            "AZURE_OPENAI_ENDPOINT=https://<resource>.openai.azure.com\n"
            "AZURE_OPENAI_API_VERSION=2024-10-21\n"
            "CI_TEXT_MODEL=gpt-4o-mini\n"
            "CI_VISION_MODEL=gpt-4o-mini",
            language="bash",
        )
