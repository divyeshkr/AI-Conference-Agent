...
if settings.provider == "mock":
    st.warning(
        "Mock mode: extraction returns realistic stand-in cards instead of reading your "
        "files. Configure a provider on the Setup page for real extraction.",
        icon="🧪",
    )
...
if pending:
    ...
    if st.button("Process and extract", type="primary", use_container_width=True):
        ...
        for i, (name, data) in enumerate(pending, start=1):
            ...
            total_cards += made
            log.write(f"✅ **{name}** — {len(artifacts)} unit(s) → {made} insight card(s)")
